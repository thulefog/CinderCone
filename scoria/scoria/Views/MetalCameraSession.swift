...
public final class MetalCameraSession: NSObject {
    ...
    public init(
        ...
        //...
        let timestamp = Date()
        let fileNameSegment = "\(timestamp.dicomDateString()).\(timestamp.dicomTimeString())"
        let outputURL = URL(fileURLWithPath: NSTemporaryDirectory()).appendingPathComponent("\(fileNameSegment).mov")
        self.recorder = MetalVideoRecorder( outputURL: outputURL, size: CGSize( width: 1080, height: 1920 ))!
    ...
    public func start() {
        ...
        captureSessionQueue.async(execute: {
            do {
                NSLog("\(#function): Starting capture session")
                ...
                //...
                print("\(#function) Start recording in session state: \(self.state)")
                self.recorder.startRecording()
            }
            catch let error as MetalCameraSessionError {
                NSLog("On start, session changed state: \(self.state) with error: \(error.localizedDescription ?? "None").")
    ...
    public func stop() {
        ...
        // OPEN: async vs sync
        captureSessionQueue.async(execute: {
            print("\(#function) Ending recording in session state: \(self.state)")
            self.recorder.endRecording( {
                //TODO: fill in empty closure
            })
    ...
    fileprivate var state: MetalCameraSessionState = .waiting {
        didSet {
            guard state != .error else {
                print( "\(#function) GUARD: request for state: \(state)" )
                return
            }

            print( "\(#function) passing request for *state: \(state) to delegate" )
...
extension MetalCameraSession: AVCaptureVideoDataOutputSampleBufferDelegate {
    ...
    public func captureOutput(_ captureOutput: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
        do {
            ...
            //DEBUG: print("\(#function): state: \(self.state)")

            //...
            if( self.state == .streaming ) {
                print("\(#function): Writing textures, N: \(textures.count) / state: \(self.state)")
                
                for texture in textures {
                    let timestamp = Date()
                    let timestampFormatted = "\(timestamp.dicomDateString()).\(timestamp.dicomTimeString())"

                    print("\(#function) TEXTURE TIMESTAMP: \(timestampFormatted) / SIZE: \(texture.width) x \(texture.height)")
