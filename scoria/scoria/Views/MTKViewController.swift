...
open class MTKViewController: UIViewController {
    ...
    open func didRenderTexture(_ texture: MTLTexture, withCommandBuffer commandBuffer: MTLCommandBuffer, device: MTLDevice) {
        ...
        let outputURL = URL(fileURLWithPath: NSTemporaryDirectory()).appendingPathComponent("\(fileNameSegment).png")
        saveTexturePNG(texture: texture, to: outputURL )
        print("Texture written: \(outputURL)")
        */
    ...
    fileprivate func initializeRenderPipelineState() {
        ...
        // MARK: SHADERS
        // NOTE: This are seed shaders from 2018
